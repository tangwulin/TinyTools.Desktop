...
<template>
    ...
    >
      ...
      <template #footer>
        <div style="display: flex;flex-direction: row-reverse">
          <n-button type="error" :disabled="confirm!=='yes'" @click="clearData">清除数据</n-button>
        </div>
      </template>
    </n-card>
  </n-modal>
  <n-space vertical>
    ...
    <p>座位信息修改</p>
    <div>
      <span style="font-size: 1rem;color: red;margin-right: 1rem;">注意：修改实时生效，不可回滚，修改错误会导致程序无法正常使用 （如下方只有null请切回座位抽选页再切回这里）</span>
    ...
    <n-collapse id="xxx">
      <n-collapse-item title="原始座位信息">
        <div class="jsoneditor">
          <JsonEditorVue v-model="allSeats" style="width: 100%;" class="jse-theme-dark mr-3" />
        </div>
      </n-collapse-item>
      <n-collapse-item title="渲染座位信息">
        <div class="jsoneditor">
          <JsonEditorVue v-model="oldRenderingList" style="width: 100%;" class="jse-theme-dark mr-3" />
        </div>
      </n-collapse-item>
    </n-collapse>
  </n-space>
