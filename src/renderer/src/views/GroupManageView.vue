...
const handler = () => {
  ...
  else
  {
    ...
    personList.value.forEach(item => {if (!('group' in item)) item.group = []})
    currentGroup.value.members.forEach(item => personList.value.find(p => p.uniqueId === item).group.push(currentGroup.value.uniqueId))
