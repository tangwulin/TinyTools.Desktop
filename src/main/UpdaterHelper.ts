...
export function printUpdaterMessage(arg, window: BrowserWindow | null) {
  const message = {
    error: '更新出错',
    checking: '正在检查更新',
  ...
  if (window) window.webContents.send('printUpdaterMessage', message[arg] ?? arg)
}

export const launchUpdater = (window: BrowserWindow | null, logger: Logger) => {
  ...
  autoUpdater.on('error', function (error) {
    printUpdaterMessage('error', window)
    if (window) window.webContents.send('updateError', error)
  })

  // 2. 开始检查是否有更新
  autoUpdater.on('checking-for-update', function () {
    printUpdaterMessage('checking', window)
  })

  // 3. 有更新时触发
  autoUpdater.on('update-available', function (info) {
    printUpdaterMessage('updateAvailable', window)
    // 4. 告诉渲染进程有更新，info包含新版本信息
    if (window) window.webContents.send('updateAvailable', info)
  })
  ...
  autoUpdater.on('update-not-available', () => {
    printUpdaterMessage('updateNotAvailable', window)
  })
  ...
  autoUpdater.on('download-progress', function (progressObj) {
    printUpdaterMessage('downloadProgress', window)
    if (window) window.webContents.send('downloadProgress', progressObj)
  })

  // 10. 下载完成，告诉渲染进程，是否立即执行更新安装操作
  autoUpdater.on('update-downloaded', function () {
    if (window) window.webContents.send('updateDownloaded')
    // 12. 立即更新安装
